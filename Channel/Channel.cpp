...
Channel::Channel(std::string name, User user)
{
    ...
    this->mode = DEFAULT;
    this->userPermit.insert(user.getfd(), USERMODE_SUPER);
}
...
{
    if (mode == DEFAULT || mode == INVITE_ONLY)
        this->mode = mode;
}
...
const std::string& Channel::getName() const {
    ...
    const UserMap& Channel::getUsers() const {
        return users;
    }

    const UserMap& Channel::getBannedUsers() const {
        return banned_user;
    }

    const std::map<int, int>& Channel::getUserPermits() const {
        return userPermit;
    }