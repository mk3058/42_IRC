...
void StatusCode::initialMap()
{
    ...
    statusMap["RPL_AWAY"] = "301"; // When an invited user is temporarily away (INVITE)
    statusMap["RPL_CHANNELMODEIS"] = "324"; // if user join channel, informs the current channel mode (MODE, JOIN)
    statusMap["RPL_UNIQOPIS"] = "325"; // Returned when the user is an operator upon inquiry. (MODE +o)
    statusMap["RPL_NOTOPIC"] = "331"; // When a user queries the topic, and the topic has not been set. (TOPIC)
    statusMap["RPL_TOPIC"] = "332"; // if topic is set and user join channel, send a topic massage (JOIN, TOPIC)
    ...
    statusMap["RPL_INVITING"] = "341"; // when invited user is successfully in channel. (INVITE)
    statusMap["RPL_INVITELIST"] = "346"; // List of users with invite privileges (MODE +i)
    statusMap["RPL_ENDOFINVITELIST"] = "347"; // after code 346, to signal completion of the inquiry (MODE +i)
    // statusMap["RPL_EXCEPTLIST"] = "348"; // when exception list is set and user check the list. (MODE +e)
    // statusMap["RPL_ENDOFEXCEPTLIST"] = "349"; // after code 348, to signal completion of the inquiry (MODE +e)
    statusMap["ERR_NOSUCHNICK"] = "401"; // nickname or channel no such in server (INVITE)
