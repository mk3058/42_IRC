...
#define RPL_UNIQOPIS "325"  // Returned when the user is an operator upon inquiry. (MODE +o)
...
#define RPL_INVITING "341"  // when the invited user is successfully in the channel. (INVITE)
#define RPL_INVITELIST "346"  // List of users with invite privileges (MODE +i)
#define RPL_ENDOFINVITELIST "347"  // after code 346, to signal completion of the inquiry (MODE +i)
...
#define ERR_NOSUCHNICK "401"  // nickname or channel not found on the server (INVITE)
...
#define ERR_CHANOPRIVSNEEDED "482"  // not the channel's operator (MODE, TOPIC, INVITE, KICK)
